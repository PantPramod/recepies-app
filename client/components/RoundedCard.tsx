...
const RoundedCard = ({ imgSrc, category }: propTypes) => {
    const router = useRouter()
    return (
        <div
            onClick={() => router.push(`/category/${category}`)}
            className='cursor-pointer hover:scale-[105%] transition-all ease-in-out duration-300  w-[200px] h-[200px] rounded-full  overflow-hidden bg-[#e9e3e398] mx-auto'>
            <img
